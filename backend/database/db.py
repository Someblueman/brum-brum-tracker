...
class AircraftDatabase:
    ...
    def __init__(self, db_path: str = "backend/aircraft_cache.db"):
        ...
        self.create_unidentified_aircraft_table()
    ...
    def create_unidentified_aircraft_table(self) -> None:
        """Create table for logging unidentified or partially identified aircraft."""
        cursor = self.connection.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS unidentified_aircraft_log (
    ...
    def log_unidentified_aircraft(self, aircraft_data: Dict[str, Any]) -> None:
        """
        Log an unidentified or partially identified aircraft for future analysis.
        
        Args:
            aircraft_data: Dictionary containing all available aircraft information
        """
        cursor = self.connection.cursor()
        cursor.execute("""
            INSERT INTO unidentified_aircraft_log (
                icao24, raw_type, simplified_type, manufacturer, type_name,
                data_source, raw_api_response, callsign, registration, 
                operator, additional_data
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            aircraft_data.get('icao24', ''),
            aircraft_data.get('raw_type', ''),
            aircraft_data.get('simplified_type', ''),
            aircraft_data.get('manufacturer', ''),
            aircraft_data.get('type_name', ''),
            aircraft_data.get('data_source', ''),
            aircraft_data.get('raw_api_response', ''),
            aircraft_data.get('callsign', ''),
            aircraft_data.get('registration', ''),
            aircraft_data.get('operator', ''),
            aircraft_data.get('additional_data', '')
        ))
        self.connection.commit()
    
    def get_unidentified_aircraft_log(self, limit: int = 100) -> List[Dict[str, Any]]:
        """
        Retrieve unidentified aircraft log entries.
        ...
        cursor = self.connection.cursor()
        cursor.execute("""
            SELECT * FROM unidentified_aircraft_log 
