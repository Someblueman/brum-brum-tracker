...
class FlightDataClient:
    ...
    def __init__(self):
        ...
        if self.use_http_fallback:
            ...
            if Config.OPENSKY_USERNAME and Config.OPENSKY_PASSWORD:
                self.api = OpenSkyApi(
                    username=Config.OPENSKY_USERNAME, password=Config.OPENSKY_PASSWORD
                )
                logger.info("Initialized OpenSky API with authentication")
            else:
                self.api = OpenSkyApi()
                logger.info(
                    "Initialized OpenSky API without authentication (rate limited)"
                )
    ...
    def _fetch_via_http(
        ...
        try:
            ...
            logger.debug(
                f"Response keys: {data.keys() if isinstance(data, dict) else 'not a dict'}"
            )
            ...
            for idx, state in enumerate(states):
                ...
                if idx < 3:
                    logger.debug(
                        f"Aircraft {idx}: {aircraft['icao24']}, "
    ...
    def _get_oauth_token(self) -> Optional[str]:
        ...
        if not Config.OPENSKY_USERNAME or not Config.OPENSKY_PASSWORD:
        ...
        data = {
            ...
            "client_id": Config.OPENSKY_USERNAME,  # This is actually the OAuth2 client_id
            "client_secret": Config.OPENSKY_PASSWORD,  # This is actually the OAuth2 client_secret
        }
    ...
    def _enforce_rate_limit(self) -> None:
        ...
        if time_since_last < Config.POLLING_INTERVAL:
            sleep_time = Config.POLLING_INTERVAL - time_since_last
    ...
    def fetch_state_vectors(
        ...
        logger.info(
            f"Fetching state vectors with bbox: min_lat={bbox[0]:.4f}, max_lat={bbox[1]:.4f}, "
        ...
        try:
            ...
            logger.info(
                f"OpenSky API response: states={states}, "
                f"has states: {states is not None and hasattr(states, 'states')}"
            )

            if not states:
                logger.warning(
                    "No states returned from OpenSky API (states is None or empty)"
                )
                return []

            if not hasattr(states, "states") or not states.states:
                logger.warning(
                    f"States object has no 'states' attribute or it's empty. "
            ...
            for idx, state in enumerate(states.states):
                ...
                if idx < 3:
                    logger.debug(
                        f"Aircraft {idx}: icao24={aircraft['icao24']}, "
    ...
    def build_bounding_box(
        self, home_lat: float, home_lon: float, radius_km: float = None
    ...
    def filter_aircraft(
        ...
        for aircraft in aircraft_list:
            ...
            distance = haversine_distance(
                home_lat, home_lon, aircraft["latitude"], aircraft["longitude"]
            )
            ...
            plane_to_home = bearing_between(
                aircraft["latitude"], aircraft["longitude"], home_lat, home_lon
            )
            ...
            if aircraft["true_track"] is not None:
                if not is_plane_approaching(
                    home_to_plane, plane_to_home, aircraft["true_track"]
                ):
                    continue
    ...
    def is_visible(self, aircraft: Dict[str, Any]) -> bool:
        ...
        angle = elevation_angle(aircraft["distance_km"], aircraft["baro_altitude"])
        aircraft["elevation_angle"] = angle

        return angle >= Config.MIN_ELEVATION_ANGLE

    def select_best_plane(
        self, visible_planes: List[Dict[str, Any]]
        ...
        sorted_planes = sorted(
            visible_planes, key=lambda p: p.get("elevation_angle", 0), reverse=True
        )

        best = sorted_planes[0]
        logger.info(
            f"Selected aircraft {best['icao24']} with elevation {best['elevation_angle']:.1f}°"
        )
...
def build_bounding_box(
    home_lat: float, home_lon: float, radius_km: float = None
) -> Tuple[float, float, float, float]:
