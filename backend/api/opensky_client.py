...
class FlightDataClient:
    ...
    def __init__(self):
        ...
        if self.use_http_fallback:
            ...
            if Config.OPENSKY_USERNAME and Config.OPENSKY_PASSWORD:
                self.api = OpenSkyApi(username=Config.OPENSKY_USERNAME, password=Config.OPENSKY_PASSWORD)
    ...
    def build_bounding_box(self, home_lat: float, home_lon: float, 
                          radius_km: float = None) -> Tuple[float, float, float, float]:
        """
    ...
    def filter_aircraft(self, aircraft_list: List[Dict[str, Any]], 
                       home_lat: float = None, 
                       home_lon: float = None) -> List[Dict[str, Any]]:
        """
